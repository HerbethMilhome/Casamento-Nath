    ...
    <div className="relative min-h-screen">
      ...
      <div className="fixed bottom-5 right-5 z-50">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-[#2C3225] hover:bg-black text-white text-xs font-semibold uppercase tracking-wider shadow-2xl transition-all hover:scale-105 border border-white/20 cursor-pointer"
          title={activeView === 'public' ? 'Abrir Painel dos Noivos' : 'Ver Site Público'}
        >
          {activeView === 'public' ? (
            <>
              <Settings size={15} className="text-[#A98C5B]" />
              <span>Área dos Noivos</span>
            </>
          ) : (
            <>
              <Eye size={15} className="text-[#A98C5B]" />
